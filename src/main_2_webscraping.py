...
# The objective of this script is to continue the data collection on the same webpages and to store this data in the original file generated by the first script
...
def crc_scraping():
    ...
    #Creating the lists that are going to hold all the information I want to extract from the selected pages.
    #Further down the info of these lists will be passed onto the excel file.
    description_df = []
    price_df = []
    rating_df = []
    comments_df = []
    ...
    #Initial loop to analyse each url of the pre-selected urls.
    for url in urls_CRC:
        #Had to inlcude this line is order to avoid robot detection by the server.
        req_CRC = Request(url, headers={'User-Agent': 'Mozilla/5.0'})
        #Opens the connection with the webpage.
        CRC_client = uReq(req_CRC)
        #Downloads the webpage information and stores on this variable.
        CRC_page = CRC_client.read()
        #Closes the connection.
        CRC_client.close()
        #Analyses the webpage as HTML.
        CRC_soup = soup(CRC_page, 'html.parser')
        #Reads and stores all the info of each container (basically each product shown on the webpage) on this variable.
